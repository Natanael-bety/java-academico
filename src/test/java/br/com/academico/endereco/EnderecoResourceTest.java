...
public class EnderecoResourceTest extends JerseyTest {
    ...
    @Test
    public void test_recuperar_lista_enderecos(){
        ...
        // Vereficar os daddos da resposta
    ...
    public void teste_criar_endereco_sem_rua(){
        String enderecoJOSON = Json.createObjectBuilder()
            .add("CEP", 49000000)
            .add("bairro", "Centro")
            .add("cidade", "Aracaju")
            .add("estado", "Sergipe")
        ...
        Response response = target("/enderecos").request().post(Entity.json(enderecoJOSON));
    ...
    public void teste_criar_endereco_com_cep_invalido() {
        ...
        assertEquals("O codigo de status HTTP da resposta deve ser 422: ", 422, response.getStatus());
        assertEquals("O tipo de conteúdo HTTP da resposta deve ser texto plano: ", MediaType.TEXT_PLAIN, response.getHeaderString(HttpHeaders.CONTENT_TYPE));
        assertTrue("O conteúdo da resposta deve conter uma mensagem de validação pré-definida: ", msg.contains("O atributo CEP deve ser inteiro e ter no mínimo 8 algaritimos."));
    }
